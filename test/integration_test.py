...
def test_whole_by_golden(golden, caplog):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        source = os.path.join(tmpdirname, "code.forth")
        input_stream = os.path.join(tmpdirname, "input.txt")
        bin_target = os.path.join(tmpdirname, "target.bin")
        mnem_target = os.path.join(tmpdirname, "target.mnem")
        ...
        with contextlib.redirect_stdout(io.StringIO()) as stdout:
            translator.main([source, bin_target, mnem_target])
            print("============================================================")
            machine.main([bin_target, input_stream])

        # Выходные данные также считываем в переменные.
        with open(mnem_target, encoding="utf-8") as file:
            code = file.read()
